...
export function TransportForm() {
	...
	async function getTransportData(currentCpp) {
		// No buscar si no hay cartaporte
		if (!currentCpp || currentCpp.trim().length === 0) return;
		const transportApi =
			"https://bckappvisitantes.azurewebsites.net/api/transports/data";
		try {
			const response = await fetch(transportApi);
			if (!response.ok) throw new Error("Error obteniendo transportes");
			const data = await response.json();

			const transport = data.find(
				(item) => item.cpp === currentCpp.toUpperCase(),
			);
	...
	// Buscar datos automáticamente cuando cambia el cartaporte
	useEffect(() => {
		getTransportData(cartaporte);
		// eslint-disable-next-line react-hooks/exhaustive-deps
	}, [cartaporte]);

	async function postTransportData(e) {
		e.preventDefault();
		if (submitting) return; // evitar doble submit
		setSubmitting(true);
		...
		try {
			...
			const formattedEntryDate = entryDate && entryDate.trim() !== "" ? formatISO(new Date(entryDate)) : null;
			const formattedExitDate = exitDate && exitDate.trim() !== "" ? formatISO(new Date(exitDate)) : null;
			...
			formData.append("fecha_ingreso", formattedEntryDate ?? currentDate);
